...
def _extract_single_ffmpeg(video_path, output_dir, interval_seconds, prefix, ffmpeg_exe):
    ...
    if duration:
        ...
        if fps:
            info += f"fps≈{fps:.2f}, "
        info += f"间隔={interval_seconds}s, 预估≈{estimated} 帧"
        print(info)
    ...
    if result.returncode != 0:
        err_lines = result.stderr.strip().splitlines()[-5:] if result.stderr else []
...
def _extract_single_cv2(video_path, output_dir, interval_seconds, prefix):
    ...
    try:
        ...
        step = max(1, int(round(float(interval_seconds) * fps)))
        ...
        if total_frames is not None:
            ...
            print(f"约 {total_frames} 帧, fps≈{fps:.2f}, 时长≈{duration:.2f}s, "
                  f"间隔≈{interval_seconds}s, 步进 {step} 帧")
        else:
            print(f"fps≈{fps:.2f}, 间隔≈{interval_seconds}s, 步进 {step} 帧")
...
def extract_frames_wrapper(input_path, output_dir, interval_seconds=3, ffmpeg_path=""):
    ...
    for index, path in enumerate(tqdm(video_files, desc="视频总进度", unit="video")):
